...
from strategy import SymbolStrategy, compute_supertrend
...
def run_bot(cfg):
    ...
    log("Risk: SL %s%% ROI, TP %s%% ROI" %
        (risk["stop_loss_roi_pct"], risk["take_profit_roi_pct"]))
    ...
    while True:
        try:
            ...
            if open_map and len(open_map) >= max_open:
                targets = [s for s in open_map if s in strategies]
            else:
                n = len(scan_order)
            ...
            for sym in targets:
                ...
                try:
                    ...
                    if len(candles) < 4:
                        continue
                    closed = candles[:-1]
                    strat.update(closed, position_side=side)
                except BybitError:
                    pass
                time.sleep(0.1)

            # Process reversal exits
            for sym, pos in list(open_map.items()):
                strat = strategies.get(sym)
                if strat and strat.has_reversal():
                    price = prices.get(sym)
                    if price:
                        broker.close_position(sym, pos, price, "SuperTrend reversal")
                        strat.clear()
                        log("REVERSAL EXIT %s @ %.6f" % (sym, price), "TRADE")
